...
export function SettingsModal({
  ...
  const validateWebhook = async () => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        setMessage({ type: 'error', text: 'Invalid webhook URL' });
      }
    } catch {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center">
      ...
      {/* Modal */}
      <div className="relative bg-[#1f1f1f] rounded-lg border border-gray-800 w-full max-w-md mx-4 p-6 animate-scale-in">
          ...
          <div className="flex items-center justify-between">
            ...
            <button
              ...
              className={`relative w-12 h-6 rounded-full transition-colors ${
                notificationEnabled ? 'bg-violet-600' : 'bg-gray-600'
              }`}
            >
              <span
