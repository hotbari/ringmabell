...
export async function POST(request: Request) {
  ...
  // Background analysis: await to ensure completion in serverless
  try {
    await Promise.all([
      analyzeAspiration(data.id, user.id, body.title, body.details),
      storeAspirationEmbedding(data.id, body.title, body.details),
    ]);
  } catch (err) {
    console.error('[aspiration-analysis] Background analysis failed:', err);
  }
