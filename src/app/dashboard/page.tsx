...
export default async function DashboardPage() {
  ...
  const [aspirationsResult, alertsResult, groupsResult, groupMembersResult] = await Promise.all([
    ...
    supabase.from('aspiration_groups').select('*').eq('user_id', user.id),
    supabase.from('aspiration_group_members').select(`*, group:aspiration_groups!inner(*)`).eq('group.user_id', user.id),
  ]);

  const aspirations = aspirationsResult.data as Aspiration[] | null;
  const alerts = (alertsResult.data as Alert[] | null) || [];
  const groups = (groupsResult.data as AspirationGroup[] | null) || [];
  const groupMembers = (groupMembersResult.data as AspirationGroupMember[] | null) || [];
    ...
    <div className="min-h-screen">
      ...
      <main className="pt-24 pb-12 px-6">
        <div className="max-w-7xl mx-auto">
          ...
          <DashboardTabs activeAspirations={activeAspirations} completedAspirations={completedAspirations} alerts={alerts} groups={groups} groupMembers={groupMembers} />
        </div>
      </main>
    </div>
  );
