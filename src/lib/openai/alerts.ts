...
- 3-4문장으로 작성`;
  ...
  switch (urgency) {
    case 'low':
      return `당신은 재밌고 똑똒한 친구예요. 장기 목표에 대해 리마인드하면서 관련 정보를 공유해주세요.
...
${toneGuide}`;
    ...
    case 'medium':
      return `당신은 재밌고 똑똒한 친구예요. 중간 점검하면서 유용한 정보를 공유해주세요.
...
${toneGuide}`;
    ...
    case 'high':
      return `당신은 재밌지만 진지한 친구예요. 마감이 다가왔다고 알려주면서 도움될 정보를 주세요.
...
${toneGuide}`;
    ...
    case 'critical':
      return `당신은 재밌지만 다급한 친구예요. ${daysUntilDeadline <= 0 ? '마감이 지났지만 아직 할 수 있다고' : '진짜 급하다고'} 알려주세요.
